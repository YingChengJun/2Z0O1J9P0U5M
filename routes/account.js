...
router.post("/checkcardid", function(req, res) {
  if (amodel.checkCard(req.body.cardid)) res.send({ status: 1 }).end();
  ...
  if (!req.session.token) {
    ...
    amodel.balance(req, function(err, ret) {
      if (err) {
        ...
        res.render("account/balance", { balance: ret[0].balance, typeOfUser: req.session.token.typeOfUser });
        console.log("123");
      }
    });
  }
